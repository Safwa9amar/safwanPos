...
import { importCategories } from "@/app/inventory/actions";
...
export function CategoryImportDialog({ isOpen, onOpenChange }: ImportDialogProps) {
  const { user } = useAuth();
  ...
  const handleImport = async () => {
    ...
    const processData = async (data: any[]) => {
        setStatus('uploading');
        const result = await importCategories(user.id, data);
        if (result.success) {
            setStatus('completed');
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (Array.isArray(data)) {
            ...
            setStatus('error');
            setErrorDetails([{row: 0, error: "JSON file must contain an array of category objects."}]);
          }
        } catch (err) {
          setStatus('error');
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a CSV or JSON file to add or update categories. The category name is used as the unique key.
          </DialogDescription>
        </DialogHeader>
        ...
        {status === 'completed' && (
            <Alert variant={errorDetails.length > 0 ? "destructive" : "default"}>
                ...
                <AlertDescription>
                    <p>{processedCount} categories processed.</p>
